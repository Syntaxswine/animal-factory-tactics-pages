...
export function applyBrush(editor,tool,x,y,edge,options={}) {
 ...
 if(tool==='guard'){if(start>=0)return 'A squad member starts here.';if(!guard&&m.guards.length>=MAX_GUARDS)return 'Maximum 46 guards (50 characters total). Remove one before placing another.';const next={x,y,z,species:options.species||'pig-foreman',weapon:options.weapon||'pistol',heading:Number(options.heading??guard?.heading??225)};if(options.outfit==='red-hats')next.outfit='red-hats';else if(guard)delete guard.outfit;if(guard)Object.assign(guard,next);else m.guards.push(next);return '';}
...
export function brushShape(tool){return ['wall','door','erase-edge'].includes(tool)?'line':['yard','floor','texture','water','bridge','woodland','void','erase-tile'].includes(tool)?'rectangle':null;}
