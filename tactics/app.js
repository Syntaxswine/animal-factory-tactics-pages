import {personalityDescription} from './personalities.js';
...
import {W,H,WEAPONS,arrangeInventory,stowWeapon,equipCutters,allocateSkill,setSneaking,setOverwatch,stepInvestigation,inventoryTransfer,turnTo,AIM_ZONES,moveGroup,key,alive,incapacitated,medicalCost,stabilizePreview,stabilize,cutPreview,cutFence,squad,guards,occupant,tile,walkable,createGame,groundTarget,attackGround,STANCES,stanceOf,setStance,movementNeighbors,navigationPath,pathCost,pathTo,move,stepMovement,previewAttack,attack,equip,reload,endTurn,stepEnemy,canControl} from './engine.js';
...
const selected=()=>s.units[s.selected],target=()=>WEAPONS[selected().weapon].blast&&terrainTarget?groundTarget(terrainTarget):s.units.find(u=>u.id===targetId&&alive(u)&&s.detected.has(u.id));
...
function drawObjects(now){for(const [id,p] of Object.entries(s.contacts||{}))if(!s.detected.has(Number(id))&&levelOf(p)===renderLevel){diamond(p.x,p.y,null,'#b4937260',0,.65);textLabel('?',p.x,p.y,'#b49372');}
 ...
 for(const obj of objects){const {x,y,type}=obj,visible=obj.visible??s.visible.has(key(x,y,renderLevel));ctx.globalAlpha=visible?1:.45;
  if(type==='prop'){art.prop(ctx,project,camera.zoom,obj);}
   ...
   ctx.save();ctx.translate(p.x,p.y+3*camera.zoom);ctx.scale(u.facing,1);if(img.complete&&img.naturalWidth)ctx.drawImage(img,-sw/2,-sh,sw,sh);else{ctx.fillStyle=color;ctx.fillRect(-sw/4,-sh,sw/2,sh);}drawFlamethrower(ctx,u,camera.zoom);ctx.restore();
   ...
   ctx.font=`bold ${9*camera.zoom}px monospace`;ctx.textAlign='center';ctx.fillStyle=color;ctx.fillText((u.team==='squad'?`${u.id+1} ${u.name}`:u.name)+(u.burningTurns?' [FIRE '+u.burningTurns+']':'')+(stanceOf(u)==='standing'?'':stanceOf(u)==='kneeling'?' [K]':' [P]'),p.x,p.y+17*camera.zoom);
 ...
 ctx.globalAlpha=1;
 for(const fire of s.fires||[])if(fire.z===renderLevel&&s.seen.has(key(fire.x,fire.y,fire.z))){diamond(fire.x,fire.y,'#ef742b88','#ffa448');const p=project(fire.x,fire.y);ctx.fillStyle='#ffc452';ctx.beginPath();ctx.moveTo(p.x-5*camera.zoom,p.y);ctx.quadraticCurveTo(p.x-8*camera.zoom,p.y-9*camera.zoom,p.x+2*camera.zoom,p.y-18*camera.zoom);ctx.quadraticCurveTo(p.x+9*camera.zoom,p.y-5*camera.zoom,p.x+5*camera.zoom,p.y);ctx.fill();}
}
...
function draw(now){if(!hover||drag||camera.zoom<.2)setCursorKind(drag?'pan':camera.zoom<.2?'overview':'default');ctx.clearRect(0,0,width,height);ctx.fillStyle='#202c29';ctx.fillRect(0,0,width,height);if(camera.zoom<.2){sprites.length=0;compositeLayers(mainCtx,viewLevel,width,height,(ink,level)=>{sectorOverview(ink,project,s,level,s.seen,level===0);for(const u of s.units)if(levelOf(u)===level&&alive(u)&&(u.team==='squad'||s.detected.has(u.id))){const p=project(u.x,u.y);ink.fillStyle=u.team==='squad'?'#ffda79':'#ff9275';ink.fillRect(p.x-2,p.y-2,4,4);}});return;}sprites.length=0;compositeLayers(mainCtx,viewLevel,width,height,(ink,level)=>{ctx=ink;renderLevel=level;try{drawTerrain(); if(showCone&&renderLevel===levelOf(selected())){const u=selected(),points=[project(u.x,u.y)];for(let a=u.heading-u.cone/2;a<=u.heading+u.cone/2;a+=3)points.push(project(u.x+60*Math.cos(a*Math.PI/180),u.y+60*Math.sin(a*Math.PI/180)));poly(points,'#e8cb6920','#e8cb6990');}drawObjects(now);}finally{ctx=mainCtx;renderLevel=viewLevel;}});ctx=mainCtx;renderLevel=viewLevel;const pointerView=[camera.x,camera.y,camera.zoom,viewLevel,width,height,s.revision].join(':');if(cursor&&!drag&&pointerView!==cursorView){cursorView=pointerView;const r=canvas.getBoundingClientRect();hitTest({clientX:r.left+cursor.x,clientY:r.top+cursor.y});updateHover();}drawPreview();drawActionCost();if(terrainAim&&hover){const q=project(hover.x,hover.y);ctx.strokeStyle='#ffac62';ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(q.x,q.y,WEAPONS[selected().weapon].blast*28*camera.zoom,WEAPONS[selected().weapon].blast*14*camera.zoom,0,0,Math.PI*2);ctx.stroke();}
 if(s.effect!==lastEffect){lastEffect=s.effect;effectUntil=now+Math.max(350,(s.effect?.sequence?.length||1)*650);}
 if(s.effect&&now<effectUntil){
  const frames=s.effect.sequence,e=frames?frames[Math.max(0,Math.min(frames.length-1,Math.floor((now-(effectUntil-frames.length*650))/650)))]:s.effect,projectShot=p=>project(p.x,p.y,(p.h-viewLevel*3)*24);
  if(e.dialogue){ctx.save();ctx.font='bold 14px sans-serif';ctx.textAlign='center';ctx.fillStyle='#17251eee';ctx.fillRect(12,70,width-24,48);ctx.fillStyle='#ffe6a2';ctx.fillText(e.dialogue,width/2,99,width-48);ctx.restore();}
  if(e.trajectories?.length){for(const shot of e.trajectories){if(Math.floor(shot.origin.h/3)!==viewLevel&&shot.z!==viewLevel)continue;const a=projectShot(shot.origin),b=projectShot(shot);ctx.strokeStyle=shot.kind==='unit'?'#ffe6a2':'#c2c5a0';ctx.lineWidth=1.5*camera.zoom;ctx.beginPath();ctx.moveTo(a.x,a.y);for(const point of shot.path||[shot]){const q=projectShot(point);ctx.lineTo(q.x,q.y);}ctx.stroke();if(shot.kind!=='range'){ctx.beginPath();ctx.arc(b.x,b.y,3*camera.zoom,0,Math.PI*2);ctx.stroke();}}}
  else if(e.az===viewLevel||e.bz===viewLevel){const a=project(e.ax,e.ay,30),b=project(e.bx,e.by,30);ctx.strokeStyle=e.incendiary?'#ff9a35':e.hit?'#ffe6a2':'#c2c5a0';ctx.lineWidth=e.incendiary?12*camera.zoom:2;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();ctx.beginPath();ctx.arc(b.x,b.y,7,0,7);ctx.stroke();}
  for(const blast of e.explosions||(e.explosion?[e.explosion]:[]))if(blast.z===viewLevel){const q=project(blast.x,blast.y,15);ctx.fillStyle='#ff851d99';ctx.beginPath();ctx.ellipse(q.x,q.y,(blast.radius||5)*28*camera.zoom,(blast.radius||5)*14*camera.zoom,0,0,Math.PI*2);ctx.fill();}
 }
}
...
function shotObstruction(p){return p.obstruction?' · '+(p.obstruction.kind==='unit'?(p.obstruction.friendly?'ALLY IN LINE: ':'UNIT IN LINE: ')+p.obstruction.name:'SHOT STRIKES '+p.obstruction.kind.toUpperCase()):'';}
function updateHover(){
 ...
 const u=selected(),edge=fenceAtCursor(s,u,cursor,project,camera.zoom,viewLevel);hoverAction=contextAction(s,u,{actorId:hoverActor,point:{...hover,z:viewLevel},edge});if(terrainAim&&WEAPONS[u.weapon].blast)hoverAction={kind:'shoot',action:'ground',point:{...hover,z:viewLevel}};setCursorKind(hoverAction.kind);
 if(hoverAction.action==='ground'){const p=previewAttack(s,u,groundTarget(hoverAction.point));$('hint').textContent=p.ok?`${WEAPONS[u.weapon].short} · ${p.cost} AP · ${p.chance}% aim${p.beyond?' · BEYOND EFFECTIVE RANGE':''} · ${p.blastRadius}-tile blast · Click twice to fire at terrain`:p.reason;return;}
 if(hoverAction.action==='target'){const t=s.units.find(t=>t.id===hoverAction.id),p=previewAttack(s,u,t,burst,aimZone);$('hint').textContent=`${t.name} / ${WEAPONS[t.weapon].short} / ${t.hp} HP · ${p.ok?`${p.chance}% aim · ${p.cost} AP${shotObstruction(p)}${p.tankChance?' · '+Math.round(p.tankChance*100)+'% tank explosion on hit':''}${p.cover?' · COVER':''}`:p.reason} · ${AIM_ZONES[aimZone].label} · ${shotConfirmation===shotKey(t)?'Click again to fire':'Click to select enemy'}`;return;}
...
function sync(){ syncClock();if(hover)updateHover(); $('sight-info').textContent=(selected().cone??120)+'° cone · 75 tile landscape / 60 tile detection maximum';$('turn').disabled=!canControl(s,selected())||!!s.queue.length;selectedIds=new Set([...selectedIds].filter(id=>alive(s.units[id])));selectedIds.add(s.selected);
 ...
 if(!w.blast){terrainAim=false;terrainTarget=null;}$('terrain-aim').hidden=!w.blast;$('terrain-aim').setAttribute('aria-pressed',terrainAim);$('aim-zone').disabled=!!w.blast;
 if(!t)targetId=null;
 ...
 $('stance-info').textContent=u.burningTurns?'ON FIRE · Running in panic · '+u.burningTurns+' turns · Cannot act':(STANCES[stanceOf(u)].moveCost+(u.sneaking?2:0))+' AP / tile · '+(s.phase==='player'?'2 AP to change stance':'Free stance changes outside combat')+' · Stand to climb';
 ...
 $('weapon-stats').textContent=`${w.name} · ${w.damage} damage\n${w.cost} AP · ${w.range} tile ${w.blast&&!w.thrown?'effective range':'range'} · ${w.mag?`${u.ammo[u.weapon]}/${w.mag} ${u.weapon==='flamethrower'?'fuel bursts':'rounds'} · ${reserve(u,u.weapon)} reserve`:'melee'}`+(u.weapon==='flamethrower'?' · Lethal flame · Tank explosion: torso hit 25%, weapon hit 90% · Fatal within 1 tile; fire within 5 tiles; panic for 3 turns':'');
 if(w.blast)$('weapon-stats').textContent+=` · ${w.blast}-tile blast · Destroys walls and props · ${w.arc?'Arcing trajectory':'Direct rocket'}`;
 ...
 $('target-info').innerHTML=t?`<b>${t.name}</b> / L${levelOf(t)+1} / ${t.ground?'Blast target':WEAPONS[t.weapon].short}<br>${p.ok?`<strong>${p.chance}%</strong> aim · ${p.cost} AP${shotObstruction(p)} · ${p.rounds>1?'3 × ':''}${p.damage} damage${p.beyond?' · HEAVY SCATTER':''}${p.tankChance?' · '+Math.round(p.tankChance*100)+'% tank explosion on hit':''}${p.coverPenalty?` · ${p.heightCover&&!p.cover?'HEIGHT COVER':'COVER'} −${p.coverPenalty}%`:''}${p.rangePenalty?` · UPHILL +${p.rangePenalty} distance`:''}`:`${p.reason}${p.cover?' · in cover':''}`}`:'Select a visible guard to inspect a shot.';
 ...
 $('squad').innerHTML=s.units.filter(u=>u.team==='squad').map(u=>`<button class="squad-card" data-unit="${u.id}" aria-pressed="${selectedIds.has(u.id)}" ${!alive(u)?'disabled':''}><span class="num">0${u.id+1}</span><img alt="" src="${unitArt(u).src}"><div class="info"><strong>${u.name}</strong><small>${alive(u)?`L${levelOf(u)+1} · ${STANCES[stanceOf(u)].label} · ${WEAPONS[u.weapon].short} · ${u.hp} HP${u.burningTurns?' · FIRE '+u.burningTurns+' turns':''}`:incapacitated(u)?(u.casualty==='stable'?'STABILIZED':`BLEEDING · ${u.bleedTurns} turns`):u.casualty==='captured'?'CAPTURED':'DEAD'}</small><div class="bar"><i style="width:${u.hp/u.maxHp*100}%"></i></div><div class="bar ap"><i style="width:${u.ap/u.maxAp*100}%"></i></div><small>${['explore','won'].includes(s.phase)?'READY':`${u.ap} / ${u.maxAp} AP`}</small></div></button>`).join('');
...
function select(id,toggle=false){terrainAim=false;terrainTarget=null;if(!alive(s.units[id]))return;if(toggle){if(selectedIds.has(id)&&selectedIds.size>1){selectedIds.delete(id);if(s.selected===id)s.selected=[...selectedIds][0];}else{selectedIds.add(id);s.selected=id;}}else{selectedIds=new Set([id]);s.selected=id;}hover=null;hoverActor=null;route=null;viewLevel=levelOf(selected());$('level').value=viewLevel;targetId=null;aimZone='torso';$('aim-zone').value=aimZone;shotConfirmation=null;burst=false;s.queue=[];sync();}
...
const terrainButton=document.createElement('button');terrainButton.id='terrain-aim';terrainButton.textContent='Aim at terrain';terrainButton.hidden=true;$('attack').after(terrainButton);terrainButton.onclick=()=>{terrainAim=!terrainAim;terrainTarget=null;targetId=null;shotConfirmation=null;sync();};
...
canvas.addEventListener('pointerdown',e=>{canvas.focus();if(e.button===2||e.button===1||e.altKey){drag={x:e.clientX,y:e.clientY};setCursorKind('pan');canvas.setPointerCapture(e.pointerId);return;}if(e.button!==0)return;if(camera.zoom<.2){message('Double-click a sector to inspect it.');return;}hitTest(e);updateHover();
 ...
 if(action.action==='ground'){const point=action.point,t=groundTarget(point);if(terrainTarget&&terrainTarget.x===point.x&&terrainTarget.y===point.y&&terrainTarget.z===point.z&&shotConfirmation===shotKey(t)){tryAttack();}else{terrainTarget=point;targetId=null;shotConfirmation=shotKey(t);sync();}return;}
