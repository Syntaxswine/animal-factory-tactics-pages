...
import {loadWeight,reserve} from './inventory.js';
...
import {W,H,WEAPONS,allocateSkill,setSneaking,setOverwatch,stepInvestigation,inventoryTransfer,turnTo,AIM_ZONES,moveGroup,key,alive,incapacitated,medicalCost,stabilizePreview,stabilize,cutPreview,cutFence,squad,guards,occupant,tile,walkable,createGame,STANCES,stanceOf,setStance,movementNeighbors,navigationPath,pathCost,pathTo,move,stepMovement,previewAttack,attack,equip,reload,endTurn,stepEnemy,canControl} from './engine.js';
...
function drawObjects(now){for(const [id,p] of Object.entries(s.contacts||{}))if(!s.detected.has(Number(id))&&levelOf(p)===renderLevel){diamond(p.x,p.y,null,'#b4937260',0,.65);textLabel('?',p.x,p.y,'#b49372');}for(const pile of s.loot||[])if(pile.items.length&&levelOf(pile)===renderLevel&&s.visible.has(key(pile.x,pile.y,renderLevel))){diamond(pile.x,pile.y,'#d5b95870','#f2d78c',0,.35);}
 ...
 for(const obj of objects){const {x,y,type}=obj,visible=obj.visible??s.visible.has(key(x,y,renderLevel));ctx.globalAlpha=visible?1:.45;
  if(type==='prop'){art.prop(ctx,project,camera.zoom,obj);}
   ...
   const walking=(s.queue[0]?.id===u.id||s.queue[0]?.group?.some(o=>o.id===u.id))||(s.phase==='enemy'&&s.units[s.enemyIndex]?.id===u.id);const pose=walking?(Math.floor(now/170)%2?'walk-a':'walk-b'):'idle',frame=characterArt(u.species,u.weapon,pose,stanceOf(u)),img=load(frame.src),sw=frame.width/4*camera.zoom,sh=64*camera.zoom;
...
function sync(){ $('sight-info').textContent=(selected().cone??120)+'° cone · 75 tile landscape / 60 tile detection maximum';$('turn').disabled=!canControl(s,selected())||!!s.queue.length;selectedIds=new Set([...selectedIds].filter(id=>alive(s.units[id])));selectedIds.add(s.selected);
 ...
 $('squad').innerHTML=s.units.filter(u=>u.team==='squad').map(u=>`<button class="squad-card" data-unit="${u.id}" aria-pressed="${selectedIds.has(u.id)}" ${!alive(u)?'disabled':''}><span class="num">0${u.id+1}</span><img alt="" src="${characterArt(u.species,u.weapon).src}"><div class="info"><strong>${u.name}</strong><small>${alive(u)?`L${levelOf(u)+1} · ${STANCES[stanceOf(u)].label} · ${WEAPONS[u.weapon].short} · ${u.hp} HP`:incapacitated(u)?(u.casualty==='stable'?'STABILIZED':`BLEEDING · ${u.bleedTurns} turns`):'DEAD'}</small><div class="bar"><i style="width:${u.hp/u.maxHp*100}%"></i></div><div class="bar ap"><i style="width:${u.ap/u.maxAp*100}%"></i></div><small>${['explore','won'].includes(s.phase)?'READY':`${u.ap} / ${u.maxAp} AP`}</small></div></button>`).join('');
...
canvas.addEventListener('pointerdown',e=>{canvas.focus();if(e.button===2||e.button===1||e.altKey){drag={x:e.clientX,y:e.clientY};canvas.setPointerCapture(e.pointerId);return;}if(e.button!==0)return;if(camera.zoom<.2){message('Double-click a sector to inspect it.');return;}hitTest(e);
 if(hoverActor!==null){const u=s.units[hoverActor];if(u.team==='squad')select(u.id,e.shiftKey);else chooseEnemy(u.id);return;}
 if(!hover)return;shotConfirmation=null;if(!(selectedIds.size>1?moveGroup(s,[...selectedIds],selected(),hover.x,hover.y,viewLevel):move(s,selected(),hover.x,hover.y,viewLevel)))message('Cannot move there. Check the path and available AP.');sync();});
...
function renderUtilities(){const u=selected();$('medical-info').textContent='Medical '+u.medical+'/100 · Stabilize '+medicalCost(u)+' AP · Medkits '+u.medkits+' · Cut fence 4 AP';const host=$('utility-actions');host.replaceChildren();for(const patient of s.units.filter(p=>p.casualty==='bleeding')){const check=stabilizePreview(s,u,patient),button=document.createElement('button');button.textContent='Stabilize '+patient.name+' · '+patient.bleedTurns+' turns · '+check.cost+' AP';button.title=check.reason;button.disabled=!check.ok;button.onclick=()=>{if(stabilize(s,selected(),patient))sync();};host.append(button);}for(const [edge,kind]of Object.entries(s.edges)){if(kind!=='fence-chainlink'||!edgeCells(edge).some(p=>p.x===u.x&&p.y===u.y&&levelOf(p)===levelOf(u)))continue;const check=cutPreview(s,u,edge),button=document.createElement('button'),other=edgeCells(edge).find(p=>p.x!==u.x||p.y!==u.y);button.textContent='Cut fence '+(other.x>u.x?'east':other.x<u.x?'west':other.y>u.y?'south':'north')+' · 4 AP';button.title=check.reason;button.disabled=!check.ok;button.onclick=()=>{if(cutFence(s,selected(),edge))sync();};host.append(button);}if(!host.children.length)host.textContent='Stand beside a wire fence or bleeding teammate to use equipment.';}
...
function renderInventory(){const u=selected(),box=$('inventory');$('bag-summary').textContent='Backpack · '+loadWeight(u).toFixed(1)+' / '+u.capacity+' kg';box.replaceChildren();const note=document.createElement('p');note.textContent=loadWeight(u).toFixed(1)+' / '+u.capacity+' kg · One weapon of each type · Transfers free';box.append(note);const button=(label,fn)=>{const b=document.createElement('button');b.textContent=label;b.onclick=()=>{if(!fn())message('Transfer unavailable: check adjacency, capacity or AP.');sync();};box.append(b);};u.pack.forEach((i,n)=>{const row=document.createElement('p');row.textContent=i.type==='weapon'?WEAPONS[i.kind].short+' · '+u.ammo[i.kind]+' loaded':i.kind+' ammunition ×'+i.count;box.append(row);if(i.type==='weapon'&&!u.slots.includes(i.kind))button('Equip secondary · 3 AP',()=>equip(s,u,i.kind));button('Drop',()=>inventoryTransfer(s,u,n,'drop'));for(const v of squad(s))if(v!==u&&v.z===u.z&&Math.abs(v.x-u.x)+Math.abs(v.y-u.y)<=1)button('Give to '+v.name,()=>inventoryTransfer(s,u,n,'give',v));});for(const pile of s.loot||[])if(levelOf(pile)===levelOf(u)&&Math.abs(pile.x-u.x)+Math.abs(pile.y-u.y)<=1&&s.visible.has(key(pile.x,pile.y,levelOf(pile)))&&(pile.x===u.x&&pile.y===u.y||!blockedEdge(s,u,pile)))pile.items.forEach((i,n)=>button('Take '+(i.type==='weapon'?WEAPONS[i.kind].short+' ('+i.rounds+' loaded)':i.kind+' ammo ×'+i.count),()=>inventoryTransfer(s,u,n,'take',pile)));}
