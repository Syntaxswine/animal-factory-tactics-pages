import {cursorStyle,contextAction,fenceAtCursor} from './cursor-actions.js';
...
function drawMovementCost(){
 const u=selected();if(hoverAction?.action!=='move'||!cursor||!hover||drag||s.queue.length||!canControl(s,u)||document.querySelector('dialog[open]')||hoverActor!==null&&s.units[hoverActor]?.team==='guard'||!route?.length)return;
 const cost=pathCost(route),combat=s.phase==='player',overBudget=combat&&cost>u.ap,estimated=route.some(p=>!s.seen.has(key(p.x,p.y,levelOf(p))));
 const label=(selectedIds.size>1?u.name+' · ':'')+(combat?`${estimated?'~':''}${cost} AP / ${u.ap} available${overBudget?' · Too far':''}`:`0 AP · ${estimated?'~':''}${cost} in combat`);
 ...
 let x=cursor.x+16,y=cursor.y+18;if(x+boxWidth>width-4)x=cursor.x-boxWidth-16;if(y+boxHeight>height-4)y=cursor.y-boxHeight-12;x=Math.max(4,Math.min(width-boxWidth-4,x));y=Math.max(4,y);
 ctx.fillStyle='#17241ff2';ctx.fillRect(x,y,boxWidth,boxHeight);ctx.strokeStyle=overBudget?'#ed8b75':'#d2ae61';ctx.strokeRect(x+.5,y+.5,boxWidth-1,boxHeight-1);ctx.fillStyle=overBudget?'#ffb5a3':'#ffe2a2';ctx.fillText(label,x+8,y+boxHeight/2,boxWidth-16);ctx.restore();
}
...
function updateHover(){
 ...
 if(hoverAction.action!=='move'){$('hint').textContent=hoverAction.label||'Wait for your squad turn.';return;}
...
canvas.addEventListener('pointerdown',e=>{canvas.focus();if(e.button===2||e.button===1||e.altKey){drag={x:e.clientX,y:e.clientY};setCursorKind('pan');canvas.setPointerCapture(e.pointerId);return;}if(e.button!==0)return;if(camera.zoom<.2){message('Double-click a sector to inspect it.');return;}hitTest(e);updateHover();
 ...
 if(action.action==='loot'){$('bag-summary').parentElement.open=true;renderInventory();$('inventory').scrollIntoView({block:'nearest'});return;}
