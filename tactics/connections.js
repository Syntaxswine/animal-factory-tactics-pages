...
export function connectionProfile(m,sx,sy,direction,type){
 ...
 if(type==='road'||type==='river'){for(const p of points){const t=tile(m,p.x,p.y,0);if(t!==(type==='road'?'ground-asphalt':'water'))continue;const seam=(horizontal?'s':'e')+':'+(p.x-(direction==='west'?1:0))+':'+(p.y-(direction==='north'?1:0));if(EDGES[m.edges[seam]]?.solid||PROPS[propAt(m,p.x,p.y,0)?.kind]?.solid)throw Error(direction+': '+type+' connection is blocked at the boundary.');}const cells=type==='road'?roads:rivers;if(cells.at(-1)-cells[0]+1!==cells.length)throw Error(direction+': use one continuous '+type+' opening.');return {type,position:cells[0],width:cells.length,level:0};}
